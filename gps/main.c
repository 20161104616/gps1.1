...
int main(int argc, const char* argv[])
{
    char a[100];
    char b[100];
    char lat[9],lon[9],spe[6],course[6],days[7],time[7], satellite[3],altituden[5],la[2],lo[2];//纬度 经度 速度 航向 日期 时间 卫星数量 海拔
    FILE *fr,*fw;
    int i;
    ...
    fprintf(fw,"纬度,南北纬,经度,东西经,速度(km/h),航向,时间,日期,卫星数量,海拔高度\n");
    while(!feof(fr))
    {
        fscanf(fr, "%s%s",a,b);//纬度
        printf("%s\n%s\n",a,b);
        for(i = 0; i < 8;i++)
        {
            lat[i] = a[i+16];
